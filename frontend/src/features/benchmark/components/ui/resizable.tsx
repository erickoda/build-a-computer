...
const ResizableGroupContext = React.createContext<ResizableGroupContextValue | null>(null)
...
function ResizablePanelGroup({
  ...
  const panelRefs = React.useRef<Map<string, React.RefObject<HTMLDivElement | null>>>(new Map())
  ...
  const startResize = React.useCallback(
    (handleIndex: number, e: React.PointerEvent) => {
      ...
      const panels = Array.from(panelRefs.current.values()).map((r) => r.current)
      ...
      const onMove = (ev: PointerEvent) => {
        ...
        const newBefore = Math.max(40, Math.min(total - 40, startBeforeSize + delta))
  ...
  return (
    <ResizableGroupContext.Provider value={{ orientation, registerPanel, unregisterPanel, startResize }}>
      <div
